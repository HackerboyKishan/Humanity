...
async function startClaiming() {
    try {
        ...
        while (true) {
            const claimPromises = privateKeys.map(async (privateKey) => {
                const account = web3.eth.accounts.privateKeyToAccount(privateKey).address;
                console.log(chalk.magenta(`🌟 Claiming reward for ${account}...`));
                await claimReward(account, privateKey);
                await new Promise(resolve => setTimeout(resolve, 5000)); // Optional: wait a bit before next claim
            });

            // Wait for all claims to complete concurrently
